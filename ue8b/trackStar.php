		...
		?>
		  <form class="form-horizontal" action="<?php $_SERVER['PHP_SELF']?>" method="POST">
	        		<div class="row form-group">
	        			<input name="newProject" class="col-sm-5 btn btn-default" type="submit" value="Neues Projekt">
	        		</div>
	      </form>
	      <?php
			...
			{?>
				<form class="form-horizontal" action="<?php $_SERVER['PHP_SELF']?>" method="POST">
	        		<div class="row form-group">
	        			<label for="newProjectName" class="col-sm-2 control-label">Name</label>
	        			<input name="newProjectName" class="col-sm-5 control-label" type="text">
	        		</div>
	        		<div class="row form-group">
	        			<label for="newProjectDesc" class="col-sm-2 control-label">Description</label>
	        			<input name="newProjectDesc" class="col-sm-5 control-label" type="text">
	        		</div>
	        		<div class="row form-group">
	        			<label for="newProjectDate" class="col-sm-2 control-label">Date</label>
	        			<input name="newProjectDate" class="col-sm-5 control-label" type="date">
	        		</div>
	        		<div class="row form-group">
	        			<input name="newProjectAktualisieren" class="btn btn-default" type="submit" value="Einfügen">
	        		</div>
	        	</form>
<?php
			...
			if(isset($_POST['newProjectAktualisieren']))
	        			{
	        				$sql="INSERT INTO project (name, description, createDate) 
	        					VALUES (:newName,:newDesc,:newDate)";

	        				$insertProj=$db->prepare("$sql");


	        				$insertProj->bindParam(':newName', $newPName, PDO::PARAM_STR);
							$insertProj->bindParam(':newDesc', $newPDesc, PDO::PARAM_STR);
							$insertProj->bindParam(':newDate', $newPDate);

							$newPName=$_POST['newProjectName'];
	        				$newPDesc=$_POST['newProjectDesc'];
	        				$newPDate=$_POST['newProjectDate'];

							$insertProj->execute();

							$countNew=$insertProj->rowCount();
							//echo "Neue Zeilen: $countNew";

					       	if($countNew == 1)
					       		echo '<p class="bg-success">Das Projekt wurde erfolgreich erstellt!</p>';
					     
					       	else
					       		echo '<p class="bg-danger">Das Projekt konnte nicht erstellt werden.</p>';
	        			}
	        ...
	        { 
	        	...
	        	{ ?>
	        		...
	        		<div class="row form-group">
	        			...
	        			<input name="editProjectName" class="col-sm-5 control-label" type="text" value="<?php echo $toChange['name'] ?>">
	        		</div>
	        		<div class="row form-group">
	        			<label for="editProjectDesc" class="col-sm-2 control-label">Description</label>
	        			<input name="editProjectDesc" class="col-sm-5 control-label" type="text" value="<?php echo $toChange['description'] ?>">
	        		</div>
	        		<div class="row form-group">
	        			<label for="editProjectDate" class="col-sm-2 control-label">Date</label>
	        			<input name="editProjectDate" class="col-sm-5 control-label" type="date" value="<?php echo $toChange['createDate'] ?>">
	        		</div>
	        		<div class="row form-group">
	        			<input name="editProjectAktualisieren" class="btn btn-default" type="submit" value="Aktualisieren">
	        			<input name="editProjectAbbrechen" class="btn btn-default" type="reset" value="Abbrechen">
	        		</div>
	        		</form>
